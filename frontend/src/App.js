...
export default function App() {
   ...
   * This function also manages API rate-limiting fallbacks.
   */
  ...
  const getTextToRender = (msg) => {
    ...
    if (msg.role === 'assistant') {
      try {
        ...
        // Not JSON, render the original content
  ...
  /**
   * Custom style object for the syntax highlighter to override the default background.
   */
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      <header className="bg-white shadow-sm p-4 sticky top-0 z-20">
        <div className="max-w-4xl mx-auto flex justify-between items-center"> {/* Header container */}
          <h1 className="text-xl font-bold text-gray-800">ChatBuddy</h1>
          <div className="hidden md:flex items-center space-x-3">
            {messages.length > 0 && <button onClick={exportChat} className="px-4 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 bg-gray-100 text-gray-600 hover:bg-gray-200">Export Chat</button>}
          ...
          {isMenuOpen && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="md:hidden absolute top-full left-0 w-full bg-white shadow-lg p-4 z-20">
              <div className="flex flex-col space-y-3">
                ...
                {messages.length > 0 && <button onClick={() => { exportChat(); setIsMenuOpen(false); }} className="w-full px-4 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 bg-gray-100 text-gray-600">Export Chat</button>}
              ...
              <div className="p-5 overflow-y-auto flex-grow space-y-6">
                ...
                <div className="p-4 bg-gray-50 rounded-lg mt-4">
                  ...
                  <p className="text-sm text-gray-600">
                    ...
                    <br />
                    <br />
                    You can add your Gemini API key for Higher Rate Limits and Context Window (offering upto 896k).
                    <br />
                    <br />
                    Basic Model Supports 8k Context Window. Advance Model Supports 64k Context Window (Default Key).
                    <br />
                    <br />
                    With the Default Server Key the RateLimits are: Basic Model (7 RPM / 500 RPD), Advance Model (3 RPM / 100 RPD).
                    <br />
                    <br />
                    With your own API Key the RateLimits are: Basic (30 RPM / 14,200 RPD), Advance (15 RPM / 1000 RPD) for FREE.
                    <br />
                    <br />
                    Aditionally the app Source Code is available on GitHub 👉<span className='font-medium'>[<a href="https://github.com/KushalRoyChowdhury/ChatBuddy" target="_blank">HERE</a>]</span>👈 . Fork It, Modify it.. I don't care. Just Star it before touching.
                    <br />
                    <br />
      ...
      <main className="flex-grow overflow-y-auto p-4 max-w-4xl mx-auto w-full" onClick={() => setIsMenuOpen(false)}>
        <div className="space-y-4">
          ...
          {messages.length === 0 ? (<div className="text-center py-12 text-gray-500 bg-white rounded-xl border max-w-2xl mx-auto"><p className="text-lg mb-3">Start a conversation</p><div className="flex justify-center gap-4 mt-4"><div onClick={() => setModel('gemma-3-27b-it')} className="p-3 bg-green-50 rounded-lg cursor-pointer"><div className="font-medium text-green-600">Basic</div></div><div onClick={() => setModel('gemini-2.5-flash-lite')} className="p-3 bg-blue-50 rounded-lg cursor-pointer"><div className="font-medium text-blue-600">Advanced</div></div></div>{systemPrompt.trim() && <div className="mt-4 p-3 bg-indigo-50 rounded-lg max-w-md mx-auto"><p className="text-sm text-indigo-700">System prompt is active.</p></div>}</div>) : (
            <AnimatePresence>
              {/* Renders each message in the chat history */}
              {messages.map((msg) => (
                <motion.div key={msg.id} initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`w-full flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-3xl rounded-2xl p-4 overflow-hidden ${msg.role === 'user' ? getUserBubbleClass(msg.model) : 'bg-white border shadow-sm text-black'}`}>
                    ...
                    {/* Displays metadata for assistant messages (model used, memory status) */}
                    {msg.role === 'assistant' && (
          ...
          {/* Empty div used as a reference to scroll to the bottom of the chat */}
          <div ref={chatEndRef} />
        </div>
      </main>
