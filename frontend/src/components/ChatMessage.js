...
const ChatMessage = React.memo(({ msg, thought, messageImageMap, getTextToRender, setShowMemories, loading, modelUsed, loadingFinished, lastBubble }) => {
  ...
  const getUserBubbleClass = (msgModel) => {
    return msgModel === 'gemini-2.5-flash-lite'
      ? 'bg-blue-100 dark:bg-blue-900 text-blue-900 dark:text-blue-100 rounded-2xl rounded-br-none shadow-sm'
      : msgModel === 'image' ? 'bg-orange-100 dark:bg-orange-900 text-orange-900 dark:text-orange-100 rounded-2xl rounded-br-none shadow-sm' : 'bg-green-100 dark:bg-green-900 text-green-900 dark:text-green-100 rounded-2xl rounded-br-none shadow-sm';
  };
  ...
  return (
    ...
    >
      <div className={`max-w-3xl rounded-2xl p-4 overflow-hidden ${msg.role === 'user' ? getUserBubbleClass(msg.model) : `${loadingFinished && msg.content.length === 0 ? 'min-h-[54dvh] ' : ''} ${msg.id === lastBubble() ? 'min-h-[54dvh] ' : ''} h-max px-0 w-full text-black dark:text-white`}`}>
