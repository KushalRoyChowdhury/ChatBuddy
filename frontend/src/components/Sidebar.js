...
const Sidebar = ({ chatSessions, activeChatId, setActiveChatId, setChatSessions, isSidebarOpen, setIsSidebarOpen, isDesktop, handleSettingsClick, handleImportClick, setShowExportOptions, setTempMemories, setUploadedImages, setMessageImageMap, loading, setShowNotAvailablePopup, setThinkingProcesses, glassMode }) => {
  ...
  const desktopSidebar = (
    <AnimatePresence>
      {isSidebarOpen &&
        <motion.div
          className="bg-gray-50 dark:bg-[rgb(10,10,10)] border-r dark:border-gray-600 shadow flex flex-col fixed top-0 left-0 z-40 h-dvh w-80"
          initial={{ x: '-100%' }}
          animate={{ x: 0 }}
          exit={{ x: '-100%' }}
  ...
  const mobileSidebar = (
    <AnimatePresence>
      {isSidebarOpen && (
        <>
          <motion.div
            className={`fixed inset-0 top-0 left-0 bottom-0 bg-black bg-opacity-50 z-50 ${glassMode ? 'backdrop-blur-[1px]' : ''}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
